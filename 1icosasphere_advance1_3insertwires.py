...
#A mesh to visualize the plane of each permutation
...
#insert a polyline for each face that will be used to extrude a circle
va_face1 = [va_permutations[0],va_permutations[3],va_permutations[8],va_permutations[0]]

curveData = bpy.data.curves.new('bones1', type='CURVE')
curveData.dimensions = '3D'
curveData.resolution_u = 2

polyline = curveData.splines.new('POLY')
polyline.points.add(len(va_face1)-1)
for i, coord in enumerate(va_face1):
    x,y,z = coord
    polyline.points[i].co = (x, y, z, 1)

curveOB = bpy.data.objects.new('bones1', curveData)
scn = bpy.context.scene
scn.objects.link(curveOB)

#Line up a circle starting at va_face1[0] and extrude it along the whole
#path
